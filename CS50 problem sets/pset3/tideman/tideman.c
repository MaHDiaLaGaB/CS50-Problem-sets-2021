    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for(int j = i + 1; j < candidate_count; j++)
        {
            if( preferences[i][j] < preferences[j][i])
            {
              ...
              //  x = pairs[pair_count].winner;
              //  y = pairs[pair_count].loser;
              //  printf("%i ",x);
              //  printf("%i\n",y);
                pair_count++;
            ...
            }else if(preferences[i][j] > preferences[j][i])
            {
                ...
                //g = pairs[pair_count].winner;
                //z = pairs[pair_count].loser;
                pair_count++;
              // printf("%i ",g);
              // printf("%i\n",z);
            }
...
void sort_pairs(void)
{
    ...
    for(int i = 0 ; i < (pair_count - 1); i++)
    {
        ...
        {
            if ( preferences[pairs[position].winner][pairs[position].loser] > preferences[pairs[j].winner][pairs[j].loser])
            {
                position = j;
            }else if(position != i)
            {
