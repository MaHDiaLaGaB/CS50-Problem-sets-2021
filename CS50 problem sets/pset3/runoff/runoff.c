...
{
    // TODO
    int theMinVote = (voter_count / 2);
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated == false && candidates[i].votes < theMinVote)
        {
            theMinVote = candidates[i].votes;
...
bool is_tie(int min)
{
    ...
    for (int i = 0 ; i < candidate_count ; i++)
    {
        //check for duplicates who still in voting
        int count = 0;
        if (candidates[i].eliminated == false && candidates[i].votes == min)
        {
            ...
            if ((count > 0 && candidates[i].eliminated == false))
            {
                return true;
            }
            //else
